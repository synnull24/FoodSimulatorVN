...
function randomSimDurations(){
  // total delivery simulation: randomized, minimum 2 minutes
  const totalSec = 120 + Math.floor(Math.random()*121); // 120–240s (2–4 min)
  const confirmingSec = Math.max(8, Math.round(totalSec*0.12));
  const pickingSec = Math.max(10, Math.round(totalSec*0.18));
  const deliveringSec = totalSec - confirmingSec - pickingSec;
  return {
    confirming: confirmingSec*1000,
    picking: pickingSec*1000,
    delivering: deliveringSec*1000,
    totalSec,
  };
}
...
function placeOrder(items, address, payment, lang){
  ...
  const simDurations = randomSimDurations();
  const etaMinutes = Math.max(2, Math.ceil(simDurations.totalSec/60));
  const order = {
    code, items:orderItems, total, address, payment, status:'confirming',
    createdAt:Date.now(), etaMinutes, simDurations, statusAt:{confirming:Date.now()},
  };
...
// ============================================================
const FALLBACK_SIM = { confirming: 15000, picking: 22000, delivering: 83000 }; // ~2min, only for pre-existing orders saved before this field existed
...
function computeOrderProgress(order){
  ...
  const sim = order.simDurations || FALLBACK_SIM;
  const elapsed = Date.now() - order.createdAt;
  const t1 = sim.confirming, t2 = t1+sim.picking, t3 = t2+sim.delivering;
  if(elapsed < t1) return {status:'confirming', frac:elapsed/t1};
  if(elapsed < t2) return {status:'picking', frac:(elapsed-t1)/sim.picking};
  if(elapsed < t3) return {status:'delivering', frac:(elapsed-t2)/sim.delivering};
...
function updateTrackingUI(order, isInit){
  ...
  if(prog.status==='delivered'){
    ...
    const sim = order.simDurations || FALLBACK_SIM;
    const total = sim.confirming + sim.picking + sim.delivering;
    const elapsed = Date.now() - order.createdAt;
    const remainMMSS = formatMMSS(Math.max(0, total-elapsed));
    if(etaEl) etaEl.textContent = `${t('eta_label')}: ${remainMMSS}`;
    if(mapEtaEl) mapEtaEl.textContent = remainMMSS;
  }
...
// ============================================================
// Persistent active-order strip — survives page navigation,
// shows the most recent non-delivered order's countdown.
// ============================================================
...
function updateActiveOrderStrip(){
  const strip = document.getElementById('activeOrderStrip');
  if(!strip) return;
  const order = STATE.orders.find(o=>o.status!=='delivered');
  if(!order){
    strip.classList.remove('show');
    document.body.classList.remove('has-order-strip');
    return;
  }
  const prog = computeOrderProgress(order);
  if(order.status !== prog.status){
  ...
  document.body.classList.add('has-order-strip');
  ...
  document.getElementById('activeOrderFill').style.width = pct+'%';
  document.getElementById('activeOrderText').textContent = `${order.code} · ${t('status_'+prog.status)}`;
  document.getElementById('activeOrderTime').textContent = formatMMSS(remainingMs);
  strip.dataset.orderCode = order.code;
}

let ACTIVE_STRIP_TIMER = null;
function startActiveOrderStripTimer(){
  updateActiveOrderStrip();
  if(ACTIVE_STRIP_TIMER) clearInterval(ACTIVE_STRIP_TIMER);
  ACTIVE_STRIP_TIMER = setInterval(updateActiveOrderStrip, 1000);
}
...
function wireOverlaysAndSheetChrome(){
  document.getElementById('activeOrderStrip').addEventListener('click', ()=>{
    const code = document.getElementById('activeOrderStrip').dataset.orderCode;
    if(code) navigateTo('tracking', {code});
  });
